...
@php
$pageTitle = 'Crear Nuevo Proyecto';
$breadcrumbs = [
...
@section('content')
<x-admin.card>
    <form action="{{ route('admin.proyectos.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="categoria"
                    ...
                    <option value="Desarrollo Web" {{ old('categoria') === 'Desarrollo Web' ? 'selected' : '' }}>Desarrollo Web</option>
                    <option value="Desarrollo Móvil" {{ old('categoria') === 'Desarrollo Móvil' ? 'selected' : '' }}>Desarrollo Móvil</option>
                    <option value="Inteligencia Artificial" {{ old('categoria') === 'Inteligencia Artificial' ? 'selected' : '' }}>Inteligencia Artificial</option>
                    <option value="Internet of Things (IoT)" {{ old('categoria') === 'Internet of Things (IoT)' ? 'selected' : '' }}>Internet of Things (IoT)</option>
                    <option value="Ciencia de Datos" {{ old('categoria') === 'Ciencia de Datos' ? 'selected' : '' }}>Ciencia de Datos</option>
                    <option value="Ciberseguridad" {{ old('categoria') === 'Ciberseguridad' ? 'selected' : '' }}>Ciberseguridad</option>
                    <option value="Blockchain" {{ old('categoria') === 'Blockchain' ? 'selected' : '' }}>Blockchain</option>
                    <option value="Realidad Virtual/Aumentada" {{ old('categoria') === 'Realidad Virtual/Aumentada' ? 'selected' : '' }}>Realidad Virtual/Aumentada</option>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="descripcion"
                          ...
                          placeholder="Describe el proyecto, sus objetivos y alcance...">{{ old('descripcion') }}</textarea>
                @error('descripcion')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="tecnologias"
                          ...
                          placeholder="Ej: React, Node.js, MongoDB, TailwindCSS...">{{ old('tecnologias') }}</textarea>
                @error('tecnologias')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="requisitos"
                          ...
                          placeholder="Requisitos técnicos, funcionales o de diseño...">{{ old('requisitos') }}</textarea>
                @error('requisitos')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
